...
This thesis makes three contributions, extending the study reported in our CGW~'26 workshop
paper~\cite{jeng2026dualmodal}. Relative to that condensed workshop paper, the thesis adds:
(a)~a full three-seed ablation grid across six fusion variants and four backbones
(Tables~\ref{tab:ucf-ablation} and~\ref{tab:xd-ablation}), where the paper reported condensed,
largely single-seed tables; (b)~per-category and failure analysis
(Sections~\ref{sec:qualitative}--\ref{sec:percat}, Appendix~\ref{app:b}) together with the full
RTFM reproduction study (Appendix~\ref{app:a}); (c)~the complete test-time-adaptation
investigation---the six-strategy search, the NORM/CORAL eliminations, and the episodic-inertness
diagnosis (Chapter~\ref{ch:06}, Section~\ref{sec:coral}); and (d)~the engineering and
reproducibility documentation (Appendices~\ref{app:d} and~\ref{app:e}).
...
\emph{degrades} performance below the visual-only baseline, establishing that learned,
input-dependent gating is what allows a weak modality to help.
...
backbones are statistically indistinguishable visual-only and SO400M emerges as the clear leader
...
variant is future work (Chapter~\ref{ch:08}).
...
mean$\pm$std over the three seeds. The complete configuration provenance and reproduction guide
appear in Appendix~\ref{app:e}.
...
Chapter~\ref{ch:07} discusses the findings and positions the system against the state of the field.
Chapter~\ref{ch:08} states limitations and future work, and Chapter~\ref{ch:09} concludes. Six
appendices provide supporting material: the RTFM reproduction investigation (Appendix~\ref{app:a}),
per-category and per-class complementarity tables (Appendix~\ref{app:b}), hyperparameter sweep
detail (Appendix~\ref{app:c}), engineering notes (Appendix~\ref{app:d}), a reproducibility guide
(Appendix~\ref{app:e}), and additional figures (Appendix~\ref{app:f}).
