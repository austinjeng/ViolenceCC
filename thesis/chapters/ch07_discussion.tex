...
frozen-backbone design costs and buys computationally. The chapter then gives
...
regime in which this work's numbers are directly comparable, and closes by
stating the limitations of the study (Section~\ref{sec:disc-limitations}).
...
for 10.7 of the 12.6 points; equal-weight late fusion underperforms,
consistent with its lack of any learned way to perform that suppression. A learned-scalar control sharpens the point:
given a single trainable mixing coefficient, the coefficient stays near
equal weighting (0.44--0.49) and the result within a point of fixed-weight
late fusion, still 10--13 AP points below gated fusion on XD-Violence ---
static reweighting, learned or not, does not suffice. The evidence that the
skeleton contributes information
...
\emph{not} regime-matched with a frozen, visual-only, frame-level system.
...
\centering
\singlespacing
\caption[Published-numbers comparison on UCF-Crime and XD-Violence]{Published-numbers comparison on UCF-Crime (frame-level ROC-AUC, \%)
...
is not regime-matched (Table~\ref{tab:sota-fair}).\par\smallskip
\textit{Per-number caveats.}
$^{a}$\,GS-MoE 91.58\,/\,82.89: confirmed from the arXiv:2508.06318 primary table
(I3D, RGB-only, no text); ICCV 2025 poster. Omitted from the fair-subset comparison
(Table~\ref{tab:sota-fair}) only
...
%% --- Fair-subset (regime-matched) table ---
\begin{table}[p]
\centering
\singlespacing
\caption[Fair-subset (regime-matched) comparison]{Fair-subset (regime-matched)
comparison. \textbf{Subset definition:}
...
captured once by this caption. \textbf{Protocol note:} our UCF-Crime numbers
use the 254-of-290-video cohort (Chapter~\ref{ch:04}) and the
$64\times64$-pixel~/~${\sim}3$-FPS frame distribution, so comparability is
scoped to the training regime, not the evaluation protocol.
\textbf{Metric:} UCF AUC $=$ frame-level ROC-AUC
...
to $+13.2$ points in the condition that most collapses the visual-language
stream). None of the higher-AUC methods above
...
the regime-matched subset (Table~\ref{tab:sota-fair}) --- fully frozen
...
the full published landscape and works through the comparable-regime read in
detail; here we restate only the conclusion that matters as a limitation.
Within the comparable regime --- frozen features, no text branch, a
single-GPU-class head --- the work is competitive on XD-Violence AP but
trails every modern method in the same frozen-feature band on UCF-Crime AUC.
We attribute the residual UCF gap to CLIP-TSA~\cite{joo2023cliptsa} --- the
closest frozen-CLIP analogue --- to the temporal self-attention and
multi-crop aggregation it adds and our minimal head omits, but we do not
isolate this experimentally and leave a controlled ablation to future work.
...
residual Giant sensitivity suggests that AP's sensitivity to the ordering
among the highest-ranked frames under XD-Violence's low anomaly prevalence,
together with its imbalanced categories, still creates a challenging
optimization landscape for the largest backbone. Practically, this means single-seed
...
Three scope caps deserve emphasis. First, the method relies on
...
is an open problem. Third, the six-strategy search that produced the method
was scored on the same 20 test conditions reported in Chapter~\ref{ch:06};
residual researcher-selection risk remains despite the label-free constraint
and the gains' reproduction on backbones, severities, and seeds unseen during
screening. All caps are disclosed wherever the corresponding gains are
reported (Chapter~\ref{ch:06}).
