...
| 25 | Paired bootstrap (gated−visual, per seed; quick 260717-77p) | UCF: all 12 per-seed 95% CIs straddle 0, P(Δ>0) 0.3405–0.848 · XD: CLIP P(Δ>0)=1.0 on all three seeds (per-seed gains +1.4/+1.7/+2.7 AP pts), SO400M CIs exclude 0 on 2/3 seeds; Base/Giant flip sign with seed (s42 −1.3/−2.1 pts with CIs<0; s2024 CIs>0; P spans 0.013–0.9845 and 0.0–0.9905) | `scripts/paired_bootstrap_fusion.py`; `results/paired_bootstrap/paired_bootstrap.csv`; `results/paired_bootstrap/summary.json` | zero-GPU from saved `eval_scores.npz`; B=2000 video-level resamples, numpy default_rng(0); MANDATORY in-script gate: per-run sklearn recompute matched `eval_metrics.json` within 1e-3; CSV `mean_delta` stored as metric fraction (×100 = points) |
