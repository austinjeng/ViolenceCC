...
| 25 | Paired bootstrap (gated−visual, per seed; quick 260717-77p) | UCF: all 12 per-seed 95% CIs straddle 0, P(Δ>0) 0.3405–0.848 · XD: CLIP P(Δ>0)=1.0 on all three seeds (per-seed gains +1.7/+1.4/+2.7 AP pts for s42/s123/s2024), SO400M CIs exclude 0 on 2/3 seeds; Base/Giant flip sign with seed (s42 −1.3/−2.1 pts with CIs<0; s2024 CIs>0; P spans 0.013–0.9845 and 0.0–0.9905) | `scripts/paired_bootstrap_fusion.py`; `results/paired_bootstrap/paired_bootstrap.csv`; `results/paired_bootstrap/summary.json` | zero-GPU from saved `eval_scores.npz`; B=2000 video-level resamples, numpy default_rng(0); MANDATORY in-script gate: per-run sklearn recompute matched `eval_metrics.json` within 1e-3; CSV `mean_delta` stored as metric fraction (×100 = points) |
| 26 | Consistency rerun (ch04 footnote; quick 260719-gvj) | 29 legacy-implementation cells retrained version-locked (clean worktree @ d2e4c2b, `scoped_dirty: false`); headline Gated Giant 82.54→82.48 (both report 82.5; Δ −0.06pp); all 3 headline seeds within row seed std (per-seed −0.13/−0.15/+0.11 vs 0.36); every ch05 qualitative conclusion preserved | `scripts/run_ucf_consistency_rerun.py`; `scripts/compare_consistency_rerun.py`; `results/_consistency_rerun/{comparison.csv,corrected_table_analysis.md}` | footnote UPGRADE only — NO published number changes; adoption of the 16/29 shifted non-headline cells is a deferred post-defense propagation decision |
| 27 | Learned-scalar late fusion (ch05 control + ch07; quick 260719-gvj) | 24 runs (8 backbone×dataset × 3 seeds); learned α = sigmoid(alpha_logit) converges 0.44–0.49 visual share (min 0.440 / max 0.485); learned vs equal-weight within ±0.9 pts; 1.9–2.5 AUC below gated on UCF, 10–13 AP below gated on XD | `scripts/run_ablations.py --queue learned_late_fusion`; `results/results-index.csv` (`*_late_fusion_*_learned_*` rows); `.planning/DEFENSE-RUNS-2026-07-17.md`; α from `results/*_late_fusion_*_learned_s*/best_model.pth` `alpha_logit` (regeneration input only, §1) | strengthens input-dependent-gating claim; static reweighting (learned or not) insufficient |
