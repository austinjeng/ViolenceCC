...
checkpoints where feasible (zero-GPU: checkpoints + cached features exist); drop if
provenance cannot be established. B-series is XD-only by necessity (documented).
