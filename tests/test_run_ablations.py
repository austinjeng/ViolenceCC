...
def test_help_has_expected_queues():
    ...
    for q in ("rtfm_gate", "phase4_main", "phase4_pooling", "phase4_seeds",
              ...
              "phase10_ucf_main", "phase10_xd_main",
              "phase10_ucf_pooling", "phase10_xd_pooling",
              "phase10_ucf_seeds", "phase10_xd_seeds"):
...
def test_queue_definitions():
    ...
    assert len(QUEUES["phase10_ucf_main"]) == 3       # clip_only, late, gated (no skeleton_only)
    assert len(QUEUES["phase10_xd_main"]) == 3
    assert len(QUEUES["phase10_ucf_pooling"]) == 2     # 2person, clip_mean
    assert len(QUEUES["phase10_xd_pooling"]) == 2
    assert len(QUEUES["phase10_ucf_seeds"]) == 2       # seeds 123, 2024
    assert len(QUEUES["phase10_xd_seeds"]) == 2
    ...
    # Quick 260717-77p: learned-scalar late-fusion queue (8 configs x 3 seeds)
    assert len(QUEUES["learned_late_fusion"]) == 24
    # Total unique specs across all queues = 358
    # (252 + 14 Phase 10 + 16 m7_visual_seeds + 52 pri1_ablations_seeds
    #  + 24 learned_late_fusion).
    all_run_names = {
        s.run_name for q in QUEUES.values() for s in q
    }
    assert len(all_run_names) == 358, f"expected 358 unique run_names, got {len(all_run_names)}"
