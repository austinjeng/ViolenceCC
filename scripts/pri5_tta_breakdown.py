#!/usr/bin/env python
"""Pri 5: per-corruption-type CORAL-TTA breakdown table + heatmap.
...
Data sources (read-only):
  results/_coral_derisk/r1full_{clip,base,so400m,giant}.json
      -> SEED-42 CANONICAL per-condition CORAL R1 deltas (20 conditions each).
         d = r1 - src, AUC points on 0-100 scale.
  results/_coral_derisk/variants/v_{bb}_s{seed}_*.json
      -> per-seed CORAL R1 per-condition (s42 train ref + s123/s2024 test ref).
  ...
  results/_tta_rerun_continual/summary_3seed.json
      -> 3-seed TENT/SAR OVERALL-AUC deltas (entropy minimisation; NOT CORAL,
...
VAR = os.path.join(CORAL, "variants")
...
def load(path):
    with open(os.path.join(CORAL, path), "r", encoding="utf-8") as f:
        return json.load(f)
...
with open(tex_path, "w", encoding="utf-8") as f:
    f.write("% Pri 5: per-corruption-type CORAL-TTA breakdown (3-seed mean delta AUC, points)\n")
    ...
    f.write("\\caption{Per-corruption-type CORAL-TTA gain (3-seed mean $\\Delta$AUC, points). "
...
with open(md_path, "w", encoding="utf-8") as f:
    f.write("# Pri 5 - Per-corruption-type CORAL-TTA breakdown\n\n")
    f.write("Substantiates the paper prose numbers (+4.8 / +13.2 / -3.2) that currently live "
            "nowhere checkable. CPU-only recompute from on-disk CORAL R1 per-condition data.\n\n")
    ...
    f.write("- **TENT/SAR overall (context only)**: `results/_tta_rerun_continual/summary_3seed.json` "
            "holds 3-seed OVERALL-AUC entropy-min deltas (NOT CORAL, NOT per-family). It does NOT "
            "contain the +13.2 gaussian value -- that comes from the CORAL variants.\n\n")
    ...
    f.write(f"- **Base seed-42 gaussian_5** = **{fmt(base_s42_g5)}** AUC pts (canonical r1full). "
            "This is the single-seed CORAL over-route the paper discloses; "
    ...
    f.write("\n> These are OVERALL-AUC entropy-min deltas (different mechanism from CORAL R1); "
