#!/usr/bin/env python
"""
Phase 9 CLIP vs SigLIP2 ViT-B/16-256 vs SigLIP2 SO400M Three-Way Backbone Comparison
=====================================================================================
Generates a three-way comparison table and grouped bar charts for the
...
OUT_DIR = PROJECT_ROOT / "results" / "phase9_charts"

# --- Run Name Mapping --------------------------------------------------------
# Maps (dataset, display_label) -> (clip_run_name, siglip2_run_name, so400m_run_name)
# skeleton_only has no SigLIP2/SO400M variant (backbone-independent)
COMPARISON_MAP = {
    # UCF main variants (s42)
    ("ucf", "Skeleton Only"): ("ucf_skeleton_only_s42", None, None),
    ...
    ("xd", "Skeleton Only"): ("xd_skeleton_only_s42", None, None),
    ("xd", "CLIP/SigLIP2/SO400M Only"): ("xd_clip_only_s42", "xd_clip_only_siglip2_s42", "xd_clip_only_so400m_s42"),
    ("xd", "Late Fusion"): ("xd_late_fusion_s42", "xd_late_fusion_siglip2_s42", "xd_late_fusion_so400m_s42"),
    ("xd", "Gated Fusion"): ("xd_gated_fusion_s42", "xd_gated_fusion_siglip2_s42", "xd_gated_fusion_so400m_s42"),
    ("xd", "GF 2-Person"): ("xd_gated_fusion_2person_s42", "xd_gated_fusion_siglip2_2person_s42", "xd_gated_fusion_so400m_2person_s42"),
    ("xd", "GF Mean-Only"): ("xd_gated_fusion_clip_mean_s42", "xd_gated_fusion_siglip2_clip_mean_s42", "xd_gated_fusion_so400m_clip_mean_s42"),
}

SEED_MAP = {
    # 3-seed stability for Gated Fusion
    ("ucf", 42): ("ucf_gated_fusion_s42", "ucf_gated_fusion_siglip2_s42", "ucf_gated_fusion_so400m_s42"),
    ("ucf", 123): ("ucf_gated_fusion_s123", "ucf_gated_fusion_siglip2_s123", "ucf_gated_fusion_so400m_s123"),
    ("ucf", 2024): ("ucf_gated_fusion_s2024", "ucf_gated_fusion_siglip2_s2024", "ucf_gated_fusion_so400m_s2024"),
    ("xd", 42): ("xd_gated_fusion_s42", "xd_gated_fusion_siglip2_s42", "xd_gated_fusion_so400m_s42"),
    ("xd", 123): ("xd_gated_fusion_s123", "xd_gated_fusion_siglip2_s123", "xd_gated_fusion_so400m_s123"),
    ("xd", 2024): ("xd_gated_fusion_s2024", "xd_gated_fusion_siglip2_s2024", "xd_gated_fusion_so400m_s2024"),
}
...
def build_comparison_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for (dataset, label), (clip_name, sig_name, so400m_name) in COMPARISON_MAP.items():
...
def build_seed_table(df: pd.DataFrame) -> pd.DataFrame:
    idx = df.set_index("run_name")
    rows = []
    for (dataset, seed), (clip_name, sig_name, so400m_name) in SEED_MAP.items():
...
def plot_comparison_bars(comp: pd.DataFrame, metric: str, title: str,
    ...
    plot_df = comp[comp[sig_col].notna() & comp[so400m_col].notna()].copy()
    if plot_df.empty:
        return
    ...
    bars1 = ax.bar(x - width, plot_df[clip_col] * 100, width,
                   label="CLIP ViT-B/16", color="#3B82F6", edgecolor="white")
    bars2 = ax.bar(x, plot_df[sig_col] * 100, width,
                   label="SigLIP2 ViT-B/16", color="#EF4444", edgecolor="white")
    bars3 = ax.bar(x + width, plot_df[so400m_col] * 100, width,
    ...
    ax.set_ylim(bottom=max(0, plot_df[[clip_col, sig_col, so400m_col]].min().min() * 100 - 10))
...
def plot_seed_stability(seed_df: pd.DataFrame, filename: str) -> None:
    ...
    for ax, dataset in zip(axes, ["UCF", "XD"]):
        ...
        ax.bar(x - width, sub[clip_col] * 100, width,
               label="CLIP", color="#3B82F6", edgecolor="white")
        ax.bar(x, sub[sig_col] * 100, width,
               label="SigLIP2", color="#EF4444", edgecolor="white")
        ax.bar(x + width, sub[so400m_col] * 100, width,
        ...
        ymin = min(sub[clip_col].min(), sub[sig_col].min(), sub[so400m_col].min()) * 100 - 2
        ax.set_ylim(bottom=ymin)

    fig.suptitle("3-Seed Stability: CLIP vs SigLIP2 vs SO400M", fontsize=TITLE_SZ,
                 fontweight="bold")
    fig.tight_layout()
    fig.savefig(OUT_DIR / filename, dpi=DPI, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    so400m_count = df["run_name"].str.contains("so400m", na=False).sum()
    print(f"SO400M runs: {so400m_count}")

    # Build comparison table
    comp = build_comparison_table(df)
    comp_path = OUT_DIR / "backbone_comparison_3way.csv"
    ...
    seed_path = OUT_DIR / "seed_stability_3way.csv"
    ...
    plot_comparison_bars(comp, "AUC",
                         "CLIP vs SigLIP2 vs SO400M — AUC Comparison",
                         "comparison_auc_3way.png")
    plot_comparison_bars(comp, "AP",
                         "CLIP vs SigLIP2 vs SO400M — AP Comparison",
                         "comparison_ap_3way.png")
    plot_seed_stability(seed_df, "seed_stability_3way.png")
