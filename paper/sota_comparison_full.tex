...
%%    (2) the 7-row fair-subset (regime-matched) table; and
...
\begin{table}[t]
\centering
\caption{Fair-subset (regime-matched) comparison. \textbf{Subset definition:}
...
captured once by this caption. \textbf{Protocol note:} this work's UCF-Crime
numbers are computed on the 254-of-290-video evaluation cohort (Section 4.1 of
the thesis) from the $64\times64$-pixel~/~${\sim}3$-FPS frame distribution, so
the evaluation cohort and input fidelity differ from the published pipelines;
comparability is scoped to the training regime, not the evaluation protocol.
